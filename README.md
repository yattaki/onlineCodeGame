...
chara.point = 100 // ステータスを変更できるポイントです。初回ブラウザ読み込み時に100になり、ゆっくりと加算されて行きます
---
...
サンプル
```javascript
// キャラクターを {x: 100, y: 200} に移動する
chara.x = 100
chara.y = 200

// キャラクターを {x: 50, y: 50} だけ移動する
chara.x += 50
chara.y += 50
```
...
ポイントを任意の値に変更する事は出来ません

    chara.point: number

サンプル
```javascript
// キャラクターの体力をポイントの半分まで加算します
// ポイントが半分消費されます
chara.hp += chara.point / 2

// 攻撃力を現在のポイントの値に変更します
// ポイントが消費され、前の攻撃力分だけ残ります
chara.power = chara.point

// 残りのポイントを速度に追加します
chara.hp += chara.point
```
...
デバッグやポイントの確認を開発者モードを開かずとも確認が出来ます
